...
def one_hot_encode(x):
    ...
    for item in x:
        one_code = np.zeros(length)
        one_code[item] = 1
        one_hot.append(one_code)
...
def flatten(x_tensor):
    ...
    flat_img_size = height * width * channel
...
def conv_net(x, keep_prob):
    ...
    conv1 = conv2d_maxpool(
        x, conv_num_outputs = 10, conv_ksize = [4,4], conv_strides = [1,1], pool_ksize = [2,2], pool_strides = [2,2])
    
    #Conv Layer2
    conv2 = conv2d_maxpool(
        conv1, conv_num_outputs=20, conv_ksize= [4,4], conv_strides= [2,2], pool_ksize= [4,4], pool_strides= [2,2])
